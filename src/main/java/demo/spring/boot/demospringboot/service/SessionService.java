...
public class SessionService {
    ...
    public Response<UserDetail> adminLogin(String userName, String password) {
        TAdminUserVo query = new TAdminUserVo();
        query.setName(userName);
        query.setPassword(password);
        List<TAdminUserVo> tAdminUserVos = tAdminUserService.queryBase(query);
        if (tAdminUserVos.size() == 0) {
            return Response.Fail("账号密码不正确!!!");
        }
        return Response.OK(new UserDetail() {
            @Override
            public String getPassword() {
    ...
    public String getAdminUserName() {
        Object sessionUserName = session.getAttribute(Constant.SESSION_ADMIN_NAME);
        if (null == sessionUserName) {
            return null;
